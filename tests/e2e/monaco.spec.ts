...
test("Monaco editor renders the challenge starter code", async ({ page }) => {
  ...
  await expect(editor.getByText(STARTER_TEXT)).toBeVisible();
});

test("user can type and modify code in the Monaco editor", async ({ page }) => {
  await page.goto(`/problems/${SLUG}`);

  const editor = page.getByRole(EDITOR_ROLE);
  await expect(editor).toBeVisible();
  await editor.fill("// user edit");

  await expect(getEditor(page).getByText("// user edit")).toBeVisible();
});
...
test("reset button restores the original starter code", async ({ page }) => {
  ...
  await expect(getEditor(page).getByText("// changed")).toBeVisible();

  const resetButton = page.getByRole("button", { name: "Reset Code" });
  await resetButton.click();

  await expect(getEditor(page).getByText(STARTER_TEXT)).toBeVisible();
  await expect(getEditor(page).getByText("// changed")).toBeHidden();
});
