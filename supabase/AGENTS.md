...
- Google and GitHub OAuth configured in `config.toml`. `client_id` in config (not a secret), `secret` via `env()` reading from `supabase/.env`. Email (magic link) works by default — captured by Mailpit. See `docs/supabase.md` "Auth providers local development".
